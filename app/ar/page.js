...
const ThreeDModel = ({ configs }) => {
  ...
  useEffect(() => {
    ...
    // --- Lighting ---
    // Softer ambient light (just a base)
    const ambientLight = new THREE.AmbientLight(0xffffff, 1.6);
    scene.add(ambientLight);

    // Key light (main light source, stronger, angled)
    ...
    scene.add(keyLight);
    const keyLight2 = new THREE.DirectionalLight(0xffffff, 3.2);
    keyLight2.position.set(10, 10,5);
    scene.add(keyLight2);

    // Fill light (weaker, opposite side to soften shadows)
    const fillLight = new THREE.DirectionalLight(0xffffff, 1.8);
    fillLight.position.set(-5, -5, 10);
    scene.add(fillLight);

    // Back/rim light (creates outline separation from background)
